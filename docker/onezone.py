...
ONP_VERSION_JSON_PACKAGES_PATH = '/etc/oz_panel/version.json'
ONP_DEFAULT_ASSUMED_VERSION = '20.02.1'  # when version.json is missing
...
def check_version_upgrade():
    """Ensure we do not start a version more than one major ahead of the previous."""
    current_onp_version = infer_current_onp_version()
    new_onp_version = infer_new_onp_version()

    current_major = parse_major(current_onp_version)
    new_major = parse_major(new_onp_version)

    if can_upgrade(current_major, new_major):
        return

    log('')
    log('Version upgrade check failed: the installed version is {} (major {}), '
        'but the last running version was {} (major {}).'.format(
        new_onp_version, new_major, current_onp_version, current_major))
    log('Upgrading by more than one major version is not supported.')
    log('Please upgrade to the previous major version first, then start this version.')
    log('')
    sys.exit(1)


def infer_current_onp_version():
    try:
        with open(ONP_VERSION_JSON_PACKAGES_PATH, 'r') as f:
            data = json.load(f)
        return data['current']
...
def parse_major(version_str):
    """Extract major version number. '20.02.1' -> 20, '25.0' -> 25."""
    return int(version_str.split('.')[0])


def can_upgrade(current_major, new_major):
    """
    Check if upgrade from prev_version_str to new_version_str is allowed.
    - To 25: both 20.x and 21.x can go (25 is compatible with 21).
    - To 26: from 21 or 25.
    - From 27 onward: at most one major step (new_major - old_major <= 1).
    """
    if new_major <= current_major:
        return True
    if new_major == 25:
        return current_major in (20, 21)
    if new_major == 26:
        return current_major in (21, 25)

    return (new_major - current_major) <= 1


def update_persisted_onp_version():
    data = {
        'previous': infer_current_onp_version(),
        'current': infer_new_onp_version()
    }

    os.makedirs(os.path.dirname(ONP_VERSION_JSON_PACKAGES_PATH), exist_ok=True)
    with open(ONP_VERSION_JSON_PACKAGES_PATH, 'w') as f:
